    ...
    def read(self):
        ...
        try:
            if abs(status.x)>level_min:
                ...
                index=  self.protocol.jogBase.queued.setJOGCmd(E_KEY.YP_DOWN if pt.x >0 else E_KEY.YN_DOWN,True)
                return status.buttons
            if abs(status.y)>level_min:
                self.protocol.jogBase.queued.setJOGCommonParams(abs(pt.y)/factor,5)
                index=  self.protocol.jogBase.queued.setJOGCmd(E_KEY.XP_DOWN if pt.y >0 else E_KEY.XN_DOWN,True)
                return status.buttons
            if abs(status.z)>level_min:
                self.protocol.jogBase.queued.setJOGCommonParams(abs(pt.z)/factor,5)
                index=  self.protocol.jogBase.queued.setJOGCmd(E_KEY.ZP_DOWN if pt.z >0 else E_KEY.ZN_DOWN,True)
                return status.buttons
            if abs(status.yaw)>level_min:
                self.protocol.jogBase.queued.setJOGCommonParams(abs(pt.yaw)/factor, 5)
                index=  self.protocol.jogBase.queued.setJOGCmd(E_KEY.RP_DOWN if pt.yaw > 0 else E_KEY.RN_DOWN, False)
                return status.buttons
